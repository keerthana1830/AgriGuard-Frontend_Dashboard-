...
import type { HistoryEntry, PurchaseLog, PlayerStats, UserSettings, UnlockedAchievement, Achievement, PesticideRating, UserData } from './types';
import { getAllUsers, saveUser, deleteUser } from './services/userService';
import { checkAchievements } from './services/achievements';
import { soundService } from './services/soundService';
import { getPesticideRatings, savePesticideRatings } from './services/pesticideService';
...
const LEVEL_NAMES = ["Seedling", "Sprout", "Green Thumb", "Crop Master", "AgriGuard"];
...
const App: React.FC<{ username: string; onLogout: () => void }> = ({ username, onLogout }) => {
  ...
  const [playerStats, setPlayerStats] = useState<PlayerStats>({ score: 0, scans: 0, level: 1, levelName: LEVEL_NAMES[0], progress: 0, totalSavings: 0, unlockedAchievements: 0 });
  ...
  const [pesticideRatings, setPesticideRatings] = useState<Record<string, PesticideRating>>({});
  ...
  const calculatePlayerStats = useCallback((
    ...
    currentUnlockedAchievements: UnlockedAchievement[]
      ...
      const progress = (scoreToNextLevel > 0) ? Math.round((scoreInLevel / scoreToNextLevel) * 100) : 100;

      const totalSavings = currentHistory.reduce((sum, entry) => {
        const savings = Math.round((100 - entry.result.infectionLevel) / 10) * SAVINGS_MULTIPLIER;
        return sum + savings;
      }, 0);
  ...
  }, [username, history, purchaseLogs, playerStats, settings, unlockedAchievements, isDataLoaded]);
  ...
  useEffect(() => {
    try {
      ...
      if(userData) {
        ...
        const loadedSettings = { ...defaultSettings, ...userData.settings };
        setHistory(loadedHistory);
        setPurchaseLogs(loadedPurchases);
        setUnlockedAchievements(loadedAchievements);
        setSettings(loadedSettings);
        setPlayerStats(calculatePlayerStats(loadedHistory, loadedPurchases, loadedAchievements));
  ...
  }, [username, calculatePlayerStats, setLanguage]);

  useEffect(() => {
    saveCurrentUserData();
  }, [history, purchaseLogs, playerStats, settings, unlockedAchievements, saveCurrentUserData]);

  const currentPlayerData = useMemo((): UserData => ({
    history,
    purchaseLogs,
    stats: playerStats,
    settings,
  ...
  useEffect(() => {
      if (!isDataLoaded) return;
      const newPlayerStats = calculatePlayerStats(history, purchaseLogs, unlockedAchievements);
      ...
      if (newlyUnlocked.length > 0) {
          ...
          setUnlockedAchievements(prev => [
              ...prev,
              ...newlyUnlocked.map(ach => ({ id: ach.id, date: new Date().toISOString() }))
          ]);
      }
  ...
  }, [history, purchaseLogs, unlockedAchievements, calculatePlayerStats, currentPlayerData, playerStats, isDataLoaded]);
  ...
  const handleLogPurchase = (purchaseData: Omit<PurchaseLog, 'id' | 'date'>) => {
  ...
  const handleRatePurchase = (purchaseId: string, rating: number) => {
    ...
    const { pesticideNameKey } = logToRate;
    const updatedRatings = { ...pesticideRatings };
    const currentRating = updatedRatings[pesticideNameKey] || { totalScore: 0, count: 0 };
    
    updatedRatings[pesticideNameKey] = {
  ...
  const handleLanguageSelect = (selectedLanguage: string) => {
      handleUpdateSettings({ language: selectedLanguage });
  ...
  const renderView = () => {
    switch (currentView) {
      case 'dashboard': return <Dashboard onNewScan={handleNewScan} setCurrentView={setCurrentView} playerStats={playerStats} onLogPurchase={handleLogPurchase} />;
      case 'history': return <History history={history} purchaseLogs={purchaseLogs} onDeleteHistory={handleDeleteHistory} onDeletePurchase={handleDeletePurchase} onRatePurchase={handleRatePurchase} />;
      case 'field': return <FieldHeatmap history={history} />;
      case 'achievements': return <Achievements unlockedAchievements={unlockedAchievements} />;
      case 'resources': return <Resources purchaseLogs={purchaseLogs} onLogPurchase={handleLogPurchase} onDeletePurchase={handleDeletePurchase} pesticideRatings={pesticideRatings} onRatePurchase={handleRatePurchase} />;
      case 'profile': return <Profile username={username} settings={settings} onUpdateSettings={handleUpdateSettings} onChangePassword={handleChangePassword} onDeleteAccount={handleDeleteAccount} onExportData={getFullUserData} />;
      default: return <Dashboard onNewScan={handleNewScan} setCurrentView={setCurrentView} playerStats={playerStats} onLogPurchase={handleLogPurchase}/>;
    }
  };
  ...
  return (
    <>
      {showLanguageModal && <LanguagePreferenceModal onLanguageSelect={handleLanguageSelect} />}
      <div className={`bg-background min-h-screen font-sans text-text-primary transition-colors duration-300 ${showLanguageModal ? 'pointer-events-none' : ''}`}>
        <Header currentView={currentView} setCurrentView={setCurrentView} onChatToggle={handleChatToggle} username={username} onLogout={onLogout} settings={settings} />
        
        <main className="container mx-auto p-4 sm:p-6 lg:p-8">
          {renderView()}
        </main>

        <Chatbot isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} onUpdateSettings={handleUpdateSettings} />
