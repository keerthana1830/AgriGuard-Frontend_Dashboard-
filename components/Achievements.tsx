...
const AchievementCard: React.FC<{ achievement: Achievement; unlocked: UnlockedAchievement | undefined }> = ({ achievement, unlocked }) => {
    ...
    return (
        ...
        `}>
            ...
            <h3 className={`font-bold text-lg ${isUnlocked ? 'text-text-primary' : 'text-text-secondary'}`}>{t(achievement.nameKey)}</h3>
            <p className="text-sm text-text-secondary flex-grow mt-1">{t(achievement.descriptionKey)}</p>
            {isUnlocked && (
                <p className="text-xs text-primary font-semibold mt-3">
                    {t('achievements_unlocked')}: {new Date(unlocked.date).toLocaleDateString()}
                </p>
            )}
        </div>
    );
...
export const Achievements: React.FC<AchievementsProps> = ({ unlockedAchievements }) => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold text-text-primary">{t('achievements_title')}</h2>
        <p className="mt-2 text-lg text-text-secondary">{t('achievements_description')}</p>
      </div>
