...
export const Dashboard: React.FC<DashboardProps> = ({ onNewScan, setCurrentView, playerStats, onLogPurchase }) => {
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 4 * 1024 * 1024) { // 4MB limit
        setError(t('dashboard_error_sizeExceeds'));
        return;
      }
      handleCloseCamera();
      resetScanState();
  ...
  const scanImage = async (dataToScan: { base64: string; mimeType: string }, previewUrl: string) => {
    ...
    try {
      const promptTemplate = t('geminiAnalyzePrompt');
      const result = await analyzePlantImage(dataToScan.base64, dataToScan.mimeType, promptTemplate, language);
      soundService.play('success');
      
      const recommendationLower = result.recommendation.toLowerCase();
      const foundPesticide = pesticides.find(p => {
          const translatedName = t(p.nameKey).toLowerCase();
          // Make sure translatedName is not empty and is present in recommendation
          return translatedName && recommendationLower.includes(translatedName);
      });
  ...
  const handleScanButtonClick = () => {
    ...
    if (!imageData || !imagePreview) {
      setError(t('dashboard_error_noImage'));
      return;
    }
    ...
    if (navigator.permissions && navigator.permissions.query) {
      try {
        ...
        if (permissionStatus.state === 'denied') {
          setError(t('dashboard_error_cameraPermissionDenied'));
          return;
        }
      } catch (err) {
    ...
    try {
      ...
      if (err instanceof DOMException) {
        if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
            ...
            setError(t('dashboard_error_noCamera'));
        } else {
            setError(t('dashboard_error_cameraAccess'));
        }
      } else {
          setError(t('dashboard_error_cameraGeneric'));
      }
    }
  };
  ...
  const handleTakePhoto = () => {
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      if (context) {
        ...
        setError(t('dashboard_error_processImage'));
        handleCloseCamera();
      }
    }
  };
  ...
  return (
    ...
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold text-text-primary">{t('dashboard_title')}</h2>
        <p className="mt-2 text-lg text-text-secondary">{t('dashboard_welcome')}</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <div className="flex flex-col items-center justify-center">
            <div className="w-full h-64 border-2 border-dashed border-gray-300 dark:border-slate-600 rounded-lg p-4 flex items-center justify-center bg-background relative overflow-hidden">
              {isCameraOpen ? (
                ...
                <div className="text-center text-text-secondary">
                  ...
                  <p className="mt-2 font-semibold">{t('dashboard_imagePreview')}</p>
                </div>
              )}
            </div>
            <button
            ...
            >
              ...
              {isScanning ? t('dashboard_analyzing') : t('dashboard_scanUploaded')}
            </button>
          </div>
          ...
          <div className="text-center md:text-left space-y-6">
            <div>
              <h3 className="text-xl font-semibold text-text-primary">{t('dashboard_step1')}</h3>
              <p className="mt-2 text-text-secondary">{t('dashboard_step1_desc')}</p>
            </div>
            ...
            >
              ...
              <div>
                <span className="font-bold text-lg text-text-primary">{t('dashboard_upload')}</span>
                <p className="text-sm text-text-secondary">{t('dashboard_upload_desc')}</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <span className="font-bold text-lg text-text-primary">{t('dashboard_useCamera')}</span>
                <p className="text-sm text-text-secondary">{t('dashboard_useCamera_desc')}</p>
              </div>
            </button>
          </div>
        </div>
      ...
      {isScanning && <Spinner message={t('dashboard_analyzing')} />}
      
      {scanResult && imagePreview && (
        <div className="mt-8">
          <h3 className="text-2xl font-bold text-text-primary mb-4">{t('dashboard_analysisComplete')}</h3>
          <ResultCard 
            result={scanResult} 
            image={imagePreview} 
