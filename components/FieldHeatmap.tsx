...
export const FieldHeatmap: React.FC<FieldHeatmapProps> = ({ history }) => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold text-text-primary">{t('field_title')}</h2>
        <p className="mt-2 text-lg text-text-secondary">{t('field_description')}</p>
      </div>
      <div className="bg-surface rounded-lg shadow-lg p-4 sm:p-8">
        <div className="grid grid-cols-4 gap-2 md:gap-4 aspect-square max-w-2xl mx-auto">
            ...
            return (
              <div key={plotId} className="relative group flex items-center justify-center">
                ...
                <div className="absolute bottom-full mb-2 w-max p-3 bg-gray-800 text-white dark:bg-slate-200 dark:text-gray-800 text-sm rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10">
                  <h4 className="font-bold text-base">{t('field_plot', { plotId })}</h4>
                  {data ? (
                    <>
                      <p>{t('field_avgInfection')} <span className="font-bold">{data.avgInfection}%</span></p>
                      <p>{t('field_scans')} <span className="font-bold">{data.scanCount}</span></p>
                    </>
                  ) : (
                    <p>{t('field_noScans')}</p>
                  )}
         ...
         <div className="flex justify-center items-center mt-8 space-x-4">
            <span className="font-semibold">{t('field_legend')}</span>
            <div className="flex items-center space-x-1">
                <div className="w-4 h-4 rounded-full bg-green-600"></div>
                <span>{t('field_healthy')}</span>
            </div>
             <div className="flex items-center space-x-1">
                <div className="w-4 h-4 rounded-full bg-yellow-400"></div>
                <span>{t('field_moderate')}</span>
            </div>
             <div className="flex items-center space-x-1">
                <div className="w-4 h-4 rounded-full bg-red-600"></div>
                <span>{t('field_high')}</span>
            </div>
        </div>
      </div>
       {history.length === 0 && (
         <div className="text-center py-16 bg-surface rounded-lg shadow-lg">
            ...
            <p className="mt-4 text-xl text-text-secondary">{t('field_noData')}</p>
            <p className="mt-2">{t('field_getStarted')}</p>
        </div>
       )}
    </div>
  );
