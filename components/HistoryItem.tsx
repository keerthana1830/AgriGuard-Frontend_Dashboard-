...
export const HistoryItem: React.FC<HistoryItemProps> = ({ entry, onDelete }) => {
  ...
  const handleDelete = () => {
    ...
    if (window.confirm(t('historyItem_confirmDeleteScan'))) {
      onDelete(id);
    }
  };
      ...
      <div className="flex-grow">
        <div className="flex justify-between items-start">
            ...
            <div className={`px-3 py-1 text-sm font-bold rounded-full text-white ${infectionLevel > 60 ? 'bg-red-500' : infectionLevel > 30 ? 'bg-yellow-500' : 'bg-green-500'}`}>
                {infectionLevel}% {t('historyItem_infected')}
            </div>
        </div>
        <div className="mt-3 text-sm space-y-2">
           <p className="flex items-start gap-2">
                <BugAntIcon className="w-5 h-5 text-text-secondary flex-shrink-0 mt-0.5" />
                <span className="font-semibold mr-1 text-text-primary">{t('historyItem_reasoning')}</span>
            ...
            <p className="flex items-start gap-2">
                ...
                <span className="font-semibold mr-1 text-text-primary">{t('historyItem_actionTaken')}</span>
