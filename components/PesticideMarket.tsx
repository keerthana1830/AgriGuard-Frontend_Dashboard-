...
export const PesticideMarket: React.FC<PesticideMarketProps> = ({ purchaseLogs, onLogPurchase, onDeletePurchase, pesticideRatings, onRatePurchase }) => {
  ...
  const handleDelete = (id: string) => {
    ...
    if (window.confirm(t('resources_purchases_confirmDelete'))) {
      onDeletePurchase(id);
    }
  };
    ...
    <>
      ...
      <div className="space-y-8">
        <div className="p-4 bg-surface rounded-lg shadow-lg">
          <h3 className="text-xl font-bold text-text-primary mb-4">{t('resources_market_title')}</h3>
          <div className="space-y-3">
            {pesticides.map((pesticide) => {
              const ratingData = pesticideRatings[pesticide.nameKey];
              const avgRating = ratingData && ratingData.count > 0 ? ratingData.totalScore / ratingData.count : 0;

              return (
              <div key={pesticide.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-background rounded-md hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors gap-2">
                <div>
                  <p className="font-semibold text-text-primary">{t(pesticide.nameKey)}</p>
                  <div className="flex items-center gap-2 mt-1">
                     <StarRating rating={avgRating} />
                     <span className="text-xs text-text-secondary">
                        ({ratingData ? ratingData.count : 0} {t('resources_market_reviews')})
                     </span>
                  </div>
                </div>
                <div className="flex items-center gap-4 w-full sm:w-auto">
                  ...
                  <button onClick={() => handleLogClick(pesticide)}
                    ...
                    aria-label={`Log purchase for ${t(pesticide.nameKey)}`}>
          ...
          <p className="mt-4 text-xs text-gray-400 dark:text-slate-500 text-center">{t('resources_market_disclaimer')}</p>
        </div>
        
        {purchaseLogs.length > 0 && (
          <div className="p-4 bg-surface rounded-lg shadow-lg">
            <h3 className="text-xl font-bold text-text-primary mb-4">{t('resources_purchases_title')}</h3>
                    ...
                    <div className="flex flex-col items-start sm:items-end">
                      {log.rating ? (
                        <>
                          ...
                          <p className="text-xs text-text-secondary mt-1">{t('resources_purchases_rated')}</p>
                        </>
                      ) : (
                         <>
                          <StarRating rating={0} onRate={(rating) => onRatePurchase(log.id, rating)} />
                          <p className="text-xs text-text-secondary mt-1">{t('resources_purchases_rate')}</p>
                        </>
                      )}
                    </div>
