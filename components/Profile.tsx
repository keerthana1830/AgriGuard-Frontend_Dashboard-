...
export const Profile: React.FC<ProfileProps> = ({
    ...
    const handlePasswordChange = async (e: React.FormEvent) => {
        ...
        if (newPassword.length < 6) {
            setPasswordError(t('profile_passwordError_length'));
            soundService.play('error');
            return;
        }
        if (newPassword !== confirmPassword) {
            setPasswordError(t('profile_passwordError_match'));
            soundService.play('error');
            return;
        }
        ...
        try {
            ...
            setPasswordSuccess(t('profile_passwordSuccess'));
            setCurrentPassword('');
            setNewPassword('');
            setConfirmPassword('');
        } catch (error) {
            soundService.play('error');
            setPasswordError(t('profile_passwordError_fail'));
        }
    };
    ...
    const handleLanguageChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setLanguage(newLang); // Update context for immediate UI change
        onUpdateSettings({ language: newLang }); // Persist the setting
    };
    ...
    const handleAvatarUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            if (file.size > 2 * 1024 * 1024) { // 2MB limit
                alert(t('profile_avatarSizeError'));
                return;
            }
    ...
    const handleDelete = () => {
        soundService.play('click');
        if (window.confirm(t('profile_confirmDeleteAccount', { username }))) {
    ...
    return (
        <div className="space-y-8 max-w-4xl mx-auto">
            <div>
                <h2 className="text-3xl font-bold text-text-primary">{t('profile_title')}</h2>
                <p className="mt-2 text-lg text-text-secondary">{t('profile_description')}</p>
            </div>

            {/* Account Info */}
            <div className="bg-surface rounded-lg shadow-lg p-6">
                <h3 className="text-xl font-semibold text-text-primary mb-4">{t('profile_accountInfo')}</h3>
                 <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6">
                    ...
                    <div>
                        <p className="text-sm text-text-secondary">{t('profile_username')}</p>
                        ...
                        <div className="mt-2 flex gap-2">
                             ...
                             <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-3 py-1.5 text-sm bg-background hover:bg-gray-200 dark:hover:bg-slate-700 rounded-md font-semibold">
                                 <ArrowUpTrayIcon className="w-4 h-4" /> {t('profile_upload')}
                             </button>
                             <button onClick={() => setIsAvatarModalOpen(true)} className="flex items-center gap-2 px-3 py-1.5 text-sm bg-background hover:bg-gray-200 dark:hover:bg-slate-700 rounded-md font-semibold">
                                 <SparklesIcon className="w-4 h-4 text-primary" /> {t('profile_generate')}
                             </button>
                        </div>
                    </div>
                </div>
            ...
            {/* Change Password */}
            <div className="bg-surface rounded-lg shadow-lg p-6">
                <h3 className="text-xl font-semibold text-text-primary mb-4">{t('profile_changePassword')}</h3>
                <form onSubmit={handlePasswordChange} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                         <input type="password" placeholder={t('profile_currentPassword')} value={currentPassword} onChange={e => setCurrentPassword(e.target.value)}
                        ...
                        <input type="password" placeholder={t('profile_newPassword')} value={newPassword} onChange={e => setNewPassword(e.target.value)}
                           className="w-full px-4 py-2 bg-background border border-gray-300 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" required />
                        <input type="password" placeholder={t('profile_confirmNewPassword')} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
                           className="w-full px-4 py-2 bg-background border border-gray-300 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" required />
                    </div>
            ...
            {/* App Settings */}
            <div className="bg-surface rounded-lg shadow-lg p-6">
                <h3 className="text-xl font-semibold text-text-primary mb-4">{t('profile_appSettings')}</h3>
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <p className="text-text-primary">{t('profile_theme')}</p>
                        <button onClick={handleThemeToggle} className="flex items-center gap-2 px-4 py-2 rounded-full bg-background hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors">
                           ...
                           <span className="font-semibold">{settings.theme === 'light' ? t('profile_theme_dark') : t('profile_theme_light')}</span>
                        </button>
                    </div>
                     <div className="flex items-center justify-between">
                        <p className="text-text-primary">{t('profile_sound')}</p>
                        <button onClick={handleSoundToggle} className="flex items-center justify-center gap-2 w-28 px-4 py-2 rounded-full bg-background hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors">
                            {settings.isSoundEnabled ? (
                                <>
                                    ...
                                    <span className="font-semibold">{t('profile_sound_on')}</span>
                                </>
                            ) : (
                                <>
                                    <SpeakerXMarkIcon className="w-5 h-5 text-text-secondary" />
                                    <span className="font-semibold">{t('profile_sound_off')}</span>
                                </>
                            )}
                        </button>
                    </div>
                    <div className="flex items-center justify-between">
                        <p className="text-text-primary">{t('language')}</p>
                        <select 
                            value={language} 
                            onChange={handleLanguageChange} 
                            className="bg-background border border-gray-300 dark:border-slate-600 text-text-primary text-sm rounded-lg focus:ring-primary focus:border-primary block w-auto p-2"
                        >
            ...
            {/* Data Management */}
            <div className="bg-surface rounded-lg shadow-lg p-6">
                <h3 className="text-xl font-semibold text-text-primary mb-4">{t('profile_dataManagement')}</h3>
