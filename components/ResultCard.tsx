...
export const ResultCard: React.FC<ResultCardProps> = ({ result, image, setCurrentView, recommendedPesticide, onLogRecommendedPurchase }) => {
  ...
  const displayPestName = pestName.toLowerCase() === 'none' ? t('resultCard_healthyPlant') : pestName;
  ...
  const handleAction = () => {
    ...
    if (actionTriggerWords.some(word => lowercasedRecommendation.includes(word))) {
      ...
      alert(t('resultCard_actionRecorded', { recommendation }));
      ...
      <div className="md:flex">
        ...
        <div className="p-6 md:p-8 flex-grow">
          <div className="flex justify-between items-start gap-4">
            <div>
              <p className={`text-sm font-semibold uppercase tracking-wide ${colorScheme.text}`}>{t('resultCard_diagnosis')}</p>
              <div className="flex items-center gap-2">
                <BugAntIcon className={`w-8 h-8 ${colorScheme.text}`} />
                <h2 className={`mt-1 text-2xl md:text-3xl font-bold ${colorScheme.text}`}>{displayPestName}</h2>
              </div>
            </div>
            <div className={`flex items-center gap-2 flex-shrink-0 px-3 py-1 text-sm font-bold rounded-full text-white ${colorScheme.button}`}>
              ...
              <span>{infectionLevel}% {t('resultCard_infected')}</span>
            </div>
          </div>
          
          <div className="mt-4">
            <p className="text-text-secondary font-semibold">{t('resultCard_levelMeter')}</p>
            ...
            <div className="flex justify-between text-xs font-medium text-gray-500 dark:text-slate-400 mt-1">
                <span>{t('resultCard_healthy')}</span>
                <span>{t('resultCard_moderate')}</span>
                <span>{t('resultCard_severe')}</span>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-4 text-text-secondary">
            <p className="flex items-start gap-3">
              ...
              <div>
                <span className="font-bold text-text-primary">{t('resultCard_reasoning')}</span>
            ...
            <p className="flex items-start gap-3">
              ...
              <div>
                <span className="font-bold text-text-primary">{t('resultCard_recommendation')}</span>
